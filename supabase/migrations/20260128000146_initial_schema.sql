...
CREATE TABLE public.profiles (
  ...
  beep_design JSONB NOT NULL DEFAULT '{}',
  buzz_design JSONB NOT NULL DEFAULT '{}',
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  CONSTRAINT fk_profiles_user_id FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
);

ALTER TABLE public.sticker_designs
  ADD CONSTRAINT fk_sticker_designs_profile_id FOREIGN KEY (profile_id) REFERENCES profiles(id) ON DELETE CASCADE;
...
CREATE TABLE public.pages (
  ...
  title TEXT NOT NULL,
  visibility page_visibility NOT NULL DEFAULT 'public',
  meta_description TEXT,
  ai_safety_check BOOLEAN NOT NULL DEFAULT false,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
...
CREATE TABLE public.stickers (
  ...
  CONSTRAINT fk_stickers_design_id FOREIGN KEY (design_id) REFERENCES sticker_designs(id) ON DELETE SET NULL
);
...
CREATE INDEX idx_stickers_design_id ON public.stickers(design_id);
...
CREATE POLICY "sticker_designs_select_anon"
